...
const matthetheark = await client.users.fetch("852608747813077052");
	const embed = new MessageEmbed()
		.setColor(message.member.displayHexColor)
		.setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, size: 2048 }))
		.setFooter("Deasn was here!", matthetheark.avatarURL({ dynamic: true }));
	...
	if (!cmd || (cmd.conf.owner && !settings.owners.includes(message.author.id)) || (cmd.conf.registery && !settings.registery.includes(message.author.id)) || !cmd.conf.enabled) return;
